# E2 revision — target-cohort normalization vs frozen single-sample reference
...
# frozen_within = referencia = media/DP das OUTRAS coortes da mesma familia tecnologica
# frozen_cross  = referencia = SCANB; choque de tecnologia somente para coortes de microarray
# Revisao 2026-08-27: orientacao prognostica fixada a priori; nunca estimada do desfecho-alvo.
suppressMessages({library(arrow); library(survival)})
project_dir <- Sys.getenv(
  "STRESS_PROJECT_DIR",
  normalizePath(".", winslash = "/", mustWork = TRUE)
)
base <- Sys.getenv("COHORT_DIR", "")
if (!nzchar(base)) {
...
concordance_multiplier <- c(CorePAM=-1, OncotypeDX=-1, GGI=-1, MammaPrint=1)
...
for(sg in names(sigs)){ w<-sigs[[sg]]
  for(co in cohorts){ d<-D[[co]]
    ...
    ww<-concordance_multiplier[[sg]]*w
    c_in<-cidx(d$time,d$event, sc_w(z_intra,ww))
...
write.csv(res, file.path(OUT,"e2_transport_fixed_orientation.csv"), row.names=FALSE)

## Reconcile against the submitted aggregate output. Equality is expected because
## intact orientation was consistent in all cohorts, but the assertion makes that
## expectation testable rather than assumed.
submitted_dir <- Sys.getenv(
  "SUBMITTED_AGGREGATE_DIR",
  dirname(project_dir)
)
submitted_path <- file.path(submitted_dir, "e2_transport.csv")
if(file.exists(submitted_path)){
  submitted <- read.csv(submitted_path, stringsAsFactors=FALSE)
  keys <- c("sig","target","plat","mode")
  cmp <- merge(submitted, res, by=keys, suffixes=c("_submitted","_fixed"), all=TRUE)
  cmp$cindex_difference <- cmp$cindex_fixed-cmp$cindex_submitted
  cmp$matches_submitted <- is.finite(cmp$cindex_difference) & abs(cmp$cindex_difference)<1e-12
  write.csv(cmp, file.path(OUT,"e2_submitted_reconciliation.csv"), row.names=FALSE)
  stopifnot(nrow(cmp)==nrow(submitted), all(cmp$matches_submitted))
}
