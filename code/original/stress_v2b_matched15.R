...
B_BOOT  <- 1     # bootstrap ja feito no v2; aqui so o pareamento
TARGET  <- 15    # tamanho pareado ao MENOR painel presente (21-gene-like: 13-15 genes)
...
write.csv(A, file.path(OUT,"v2b_envelope_abs_neff15.csv"), row.names=FALSE)
write.csv(B, file.path(OUT,"v2b_matched_size15.csv"),     row.names=FALSE)
write.csv(C, file.path(OUT,"v2b_bootstrap_env15.csv"),    row.names=FALSE)
log("\n== salvos v2b_envelope_abs_neff15.csv (%d) / v2b_matched_size15.csv (%d) / v2b_bootstrap_env15.csv (%d)",
